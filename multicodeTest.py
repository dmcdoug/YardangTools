...
from ZonHistIterator_parTest import ZonHistIterator
...
def doWork(curvPolygs, Curvature_Raster, Input_Raster_or_Mosaic_Dataset, shpScratch, oid):
    ...
    try:
        ...
        try:
            ...
            return "Failed to mkdir for "+str(oid)
...
def multi(curvPolygs, Curvature_Raster, Input_Raster_or_Mosaic_Dataset, shpScratch):
    ...
    try:
        ...
        results = pool.map(func, idList)#,callback=complete.append,error_callback=incomplete.append).get(timeout=20) #.map_async
